...
- **Transcrição pelo navegador** (Web Speech API): custo zero de STT no web. ⚠️ **Correção (ago/2026):** a documentação anterior dizia que o áudio nunca saía do dispositivo — está errado. O navegador envia o áudio ao servidor do fornecedor (Google no Chrome, Apple no Safari) e devolve o texto. Para o Grão só o texto trafega, mas o áudio já saiu antes disso. Relato de fé é **dado sensível na LGPD**: isso precisa constar da política de privacidade antes de abrir para usuários reais. Alternativa real de on-device: `expo-speech-recognition` no app nativo, com reconhecimento local.
